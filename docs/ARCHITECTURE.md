...
flowchart TD
    ...
    REF -->|"verified + approved -> execute"| PROD
```
...
Request
  ...
  |-- Step 5: generative UI card + STOP       <- harness gate pauses here
...
| `dynamic_sub_agents` | 2 | Parallel read-only blast-radius mapping |
| `simulate_operation` (MCP) | 4 | Real clone, real impact, real rollback proof |
| `generative_ui` | 5 | Structured approval card (not a markdown wall) |
| TrueForge approval gate | 5-6 | Harness pauses run; human must click Allow |
| `refusalReason()` (code) | 6 | Second gate: refuses if simulation not verified |
...
layer is bypassed -- `refusalReason()` in `mcp-server/src/simulate.ts` reads the
in-process simulation store and returns a refusal string unless
`rollbackVerified: true` is set on the referenced simulation id. Prompts cannot
write to that store.
